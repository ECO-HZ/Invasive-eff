...
ggplot() +
  geom_point(data_Tave, mapping = aes(x = Latitude, y = Tave, fill = Year),
  ...
  scale_color_manual(values = c("#F2A622","#9A5747","#0B6374")) + 
  scale_fill_manual(values = c("#F2A622","#9A5747","#0B6374")) +
...
ggplot() +
  geom_point(data_Prec, mapping = aes(x = Latitude, y = Prec, fill = Year),
             position = position_dodge(width = 0), size = 2.5, pch = 21) + 
...
ggplot() +
  geom_point(data_Soil_ph, mapping = aes(x = Latitude, y = Soil_ph, fill = Year, group = Year),
             position = position_dodge(width = 0), size = 2.5, pch = 21) +  
...
mod4 = lm(Soil_ph ~ Year * Latitude, data = data_Soil_N)
anova(mod4)
mod4_emtrends <- emtrends(mod4, pairwise ~ Year, var = "Latitude")
test(mod4_emtrends, adjust = "BH")

ggplot() +
  geom_point(data_Soil_N, mapping = aes(x = Latitude, y = Soil_N, fill = Year, group = Year),
             position = position_dodge(width = 0), size = 2.5, pch = 21) +  
...
ggplot() +
  geom_point(data_Wcont, mapping = aes(x = Latitude, y = Wcont, fill = Year, group = Year),
             position = position_dodge(width = 0), size = 2.5, pch = 21) +  
  ...
  #scale_x_continuous(expand = expansion(mult = c(0.1, 0.1))) +
  #scale_x_continuous(breaks=c(23.1,25.2,27.9,30.5,34.6,36.2)) + 
  labs(x = NULL, y = "Soil water content (g/g, sqrt)", tag = "e") -> p5; p5
