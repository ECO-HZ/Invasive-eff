...
ggplot(data = with_bc_data_add_mean_all, 
  ...
  geom_errorbar(aes(x = Origin, ymax = mean_env + se_env*1.96, ymin = mean_env - se_env*1.96),
...
ggplot(data = with_bc_data_mean_merg, aes(x = Tave, y = diff)) + 
  ...
  annotate("text", x = 18.95, y = 0.62, label = "All sites:", size = 4) + 
  theme_bw() + mytheme + 
