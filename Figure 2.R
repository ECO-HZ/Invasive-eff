...
ggplot()+
  ...
  geom_abline(intercept = -0.1287923, slope = 1.3337283, linetype = 2, size = 1) +
  geom_point(β_Bray_Native, mapping = aes(x = Green_mean,y = Field_mean, shape = Year, fill = Site, color = Site), size = 2.5) +
  ...
  scale_x_continuous(labels = scales::label_comma(accuracy =0.01), limits = c(0.54,0.92)) +
  ...
  labs(x = NULL, 
       ...
       tag = "b", title = c("Among native plants")) -> Figure_2b; Figure_2b


pair_BC_sorted_merge_nat2 = pair_BC_sorted_merge_nat %>%
  tidyr::pivot_longer(cols = c(Field_dist, Green_dist),
                      names_to = "Group",
                      values_to = "Distance") %>%
  mutate(Group = recode(Group,
                        Field_dist = "Field",
                        Green_dist = "Greenhouse"))


mean_Bray_Native <- pair_BC_sorted_merge_nat2 %>%
  group_by(Group) %>%
  summarise(
    mean = mean(Distance, na.rm = TRUE),
    sd   = sd(Distance, na.rm = TRUE),
    se   = sd / sqrt(sum(!is.na(Distance))),
    .groups = "drop")

colnames(pair_BC_sorted_merge_nat2)[9] = "mean"
mean_Bray_Native$Group <- as.factor(mean_Bray_Native$Group)

pair_BC_sorted_merge_nat2$Group <- as.factor(pair_BC_sorted_merge_nat2$Group)
ggplot()+
  geom_density_ridges(data = pair_BC_sorted_merge_nat2, aes(y = Group, x = mean, group = Group),
                      rel_min_height = 0.01, scale = 0.3, alpha = 0.5,
                      quantile_lines = TRUE, quantile_fun = mean) +
  coord_flip() + 
  theme_bw() + mytheme + 
  annotate("segment", y = 1.1, yend = 1.1, x = 0.25, xend = 0.35, colour = "black", 
           arrow = arrow(length = unit(0.2, "cm"), type = "open", angle = 20)) + 
  annotate("text", y = 1.5, x = 0.30, label = "13.9%", size = 4) + 
  annotate("text", y = 1.5, x = 0.20, label = expression(italic("p") < "0.001"), size = 4) + 
  theme(legend.position = "none", 
        axis.text.x = element_text(angle = 30, vjust = 1, hjust = 1)) +
  labs(x = NULL, y = NULL) -> Figure_2bs; Figure_2bs

Figure_2b + annotation_custom(grob=ggplotGrob(Figure_2bs), ymin = 0.53, ymax = 0.79, xmin=0.80, xmax=0.94) -> Figure_2b; Figure_2b
...
ggplot()+
  ...
  geom_abline(intercept = -3.030977, slope = 5.517322, linetype = 2, size = 1)+
  geom_point(β_Bray_Exotic, mapping = aes(x = Green_mean,y = Field_mean, shape = Year, fill = Site, color = Site), size = 2.5) +
  ...
  labs(x = "Mean pair-wise Bray–Curtis\ndissimilaritiesestimated in the greenhouse experiment", 
       y = "", title = c("Among alien plants"), tag = "b") -> Figure_2c; Figure_2c


pair_BC_sorted_merge_exo2 = pair_BC_sorted_merge_exo %>%
  tidyr::pivot_longer(cols = c(Field_dist, Green_dist),
                      names_to = "Group",
                      values_to = "Distance") %>%
  mutate(Group = recode(Group,
...
mean_Bray_Exotic <- pair_BC_sorted_merge_exo2 %>%
  group_by(Group) %>%
  summarise(
    mean = mean(Distance, na.rm = TRUE),
    sd   = sd(Distance, na.rm = TRUE),
...
colnames(pair_BC_sorted_merge_exo2)[9] = "mean"
mean_Bray_Exotic$Group <- as.factor(mean_Bray_Exotic$Group)

pair_BC_sorted_merge_exo2$Group <- as.factor(pair_BC_sorted_merge_exo2$Group)

ggplot()+
  geom_density_ridges(data = pair_BC_sorted_merge_exo2, aes(y = Group, x = mean, group = Group),
                      rel_min_height = 0.01, scale = 0.3,alpha = 0.5, 
                      quantile_lines = TRUE, quantile_fun = mean) +
  coord_flip() + 
  annotate("segment", y = 1.1, yend = 1.1, x = 0.25, xend = 0.35, colour = "black", 
           arrow = arrow(length = unit(0.2, "cm"), type = "open", angle = 20)) + 
  annotate("text", y = 1.5, x = 0.30, label = "6.6%", size = 4) + 
  annotate("text", y = 1.5, x = 0.20, label = expression(italic("p") < "0.001"), size = 4) + 
  theme_bw() + mytheme + 
  theme(legend.position = "none", 
        axis.text.x = element_text(angle = 30, vjust = 1, hjust = 1)) +
  labs(x = NULL, y = NULL) -> Figure_2cs; Figure_2cs

Figure_2c + annotation_custom(grob=ggplotGrob(Figure_2cs), ymin = 0.53, ymax = 0.79, xmin=0.80, xmax=0.94) -> Figure_2c; Figure_2c
