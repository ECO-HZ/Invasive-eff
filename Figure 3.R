...
ggplot(data = with_bc_data_add_mean, aes(x = Tave, y = mean_env, fill = Origin, color = Origin, group = Origin)) + 
  ...
  labs(x = expression("Annual mean temperature ("*degree*C*")"),
       y = bquote(atop("Environmental effects", 
                       Ln ~ "(" ~ frac(Pairwise~dissimilarity["estimated in field survey"], 
                                       Pairwise~dissimilarity["estimated in greenhouse exp."]) ~ ")")), tag = "a",
       title = NULL) -> p1; p1
...
ggplot(data = with_bc_data_mean_merg, aes(x = Tave, y = diff)) + 
  ...
  geom_hline(yintercept = 0, linetype = 2, size = 0.5) +
  labs(x = expression("Annual mean temperature ("*degree*C*")"),
       y = "Change in environmental effects\nbetween native and alien species", tag = "b",
       title = NULL) -> p2; p2
